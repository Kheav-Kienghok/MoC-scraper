...
class KhmerEnglishAligner:
    ...
    Attributes:
        model_name (str): Hugging Face model name for multilingual sentence embeddings.
        acceptable_negative_diff (float): Minimum acceptable drop in similarity score when merging sentences.
    """

    def __init__(self, model_name: str = 'sentence-transformers/LaBSE', acceptable_negative_diff: float = -0.01):
        """
    ...
    def align(
        ...
        """
        Align Khmer sentences to English ones and optionally write results to CSV.

        Args:
            data (Dict[str, List[str]]): Dictionary with 'english' and 'khmer' sentence lists.
            output_csv_path (str): Path to output CSV file for saving sentence alignments.

        Returns:
            List[Tuple[str, str, float]]: List of (English, Khmer, Similarity) tuples.
        """
        ...
        khmer_embeddings = self.model.encode(khmer_sentences, convert_to_tensor=True)
        ...
        for km_idx, km_emb in enumerate(khmer_embeddings):
            max_score = -1
            best_en_idx = -1
            for en_idx, en_emb in enumerate(english_embeddings):
                score = util.pytorch_cos_sim(en_emb, km_emb).item()
                if score > max_score:
                    max_score = score
                    best_en_idx = en_idx
            pairs.append((best_en_idx, km_idx, max_score))
            used_english_indices.add(best_en_idx)

        # Step 2: Attempt to merge unused English sentences
        merged_english_texts = [english_sentences[p[0]] for p in pairs]
        unused_english_indices = [i for i in range(len(english_sentences)) if i not in used_english_indices]

        for unused_en_idx in unused_english_indices[:]:  # Copy for safe iteration
            unused_text = english_sentences[unused_en_idx]
            unused_emb = english_embeddings[unused_en_idx]
            merged = False

            for pair_idx, (en_idx, km_idx, old_score) in enumerate(pairs):
                existing_text = merged_english_texts[pair_idx]
                merged_text = existing_text + " " + unused_text

                # Recompute merged embedding
                merged_emb = self.model.encode(merged_text, convert_to_tensor=True)
                km_emb = khmer_embeddings[km_idx]
                new_score = util.pytorch_cos_sim(merged_emb, km_emb).item()
                diff = new_score - old_score

                if new_score > old_score or diff >= self.acceptable_negative_diff:
        ...
        aligned_english = [merged for merged in merged_english_texts]
        aligned_khmer = [khmer_sentences[km_idx] for _, km_idx, _ in pairs]
